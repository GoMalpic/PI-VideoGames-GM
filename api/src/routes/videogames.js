...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (name) {
      let promiseAPI = axios.get(
        `https://api.rawg.io/api/games?search=${name}&key=${API_KEY}`
      ); //Devuelve 20 entradas de la API
      ...
      Promise.all([promiseAPI, promiseDB]).then((response) => {
        ...
        if (dataAPI.data.length === 0 && dataDB.length === 0)
          res.send({ res: "Ninguna coincidencia" });
        const selectDataAPI = dataAPI.data.results.map(
      ...
      Promise.all(dataPromise).then((response) => {
        for (const item of response) {
          let newData = item.data.results.map(
            ({ id, name, background_image, rating, genres }) => ({
              id,
              name,
              image: background_image,
